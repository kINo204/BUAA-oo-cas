...
public class Term implements Calc {
    ...
    public boolean mergeWith(Calc next) {
        ...
        factors.addAll(((Term) next).factors);
        return true;
    }

    /**
     * @return an expression generated by diffing the term.
     */
    @Override
    public Calc diff() {
        Expr expr = new Expr();
        // Note: factors MUSTN'T be changed during loop!
        for (Factor f1 : factors) {
            // Diff current factor
            Term term = new Term();
            term.mergeWith(f1.diff()); //TODO
            // Mult with other factors to form a term
            for (Factor f2 : factors) {
                if (f1 != f2) {
                    term.addFactor((Factor) f2.cloneSubTree());
                }
            }
            // Set operator as the same
            term.optTerm = optTerm;
            term.optFact = optFact;
            // Add this term into the expr
            expr.addTerm(term);
        }
        return expr;
    }
    ...
    /**
     * Note: The terms must have at most ONE num factor!
     * @param next
     * @return
     */
